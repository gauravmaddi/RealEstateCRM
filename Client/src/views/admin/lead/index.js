...
const Index = () => {
    ...
    const changeStatus = (cell) => {
        switch (cell.original.leadStatus) {
            ...
            case "sold": return "onHold";
            default: return "completed";
        }
    };

    // Add the handleDeleteLead function within component scope
    const handleDeleteLead = async (ids) => {
        try {
            setIsLoding(true);
            const response = await deleteManyApi("api/lead/deleteMany", ids);
            if (response.status === 200) {
                setSelectedValues([]);
                setDelete(false);
                setAction(prev => !prev);
    ...
    const fetchCustomDataFields = async () => {
        ...
        try {
            ...
            if (result.payload.status === 200) {
                ...
                // Create dynamic columns from custom fields
                const customColumns = result?.payload?.data[0]?.fields
                    ?.filter(field => field?.isTableField === true)
                    ?.map(field => {
                        if (field?.name === "leadStatus") return null;
                        
                        return {
                            Header: field?.label,
                            accessor: field?.name,
                            cell: field?.isView ? ({ row, value }) => (
                                <div className="selectOpt">
                                    <Text
                                        onClick={() => navigate(`/leadView/${row?.original?._id}`)}
                                        me="10px"
                                        sx={{ '&:hover': { color: 'blue.500', textDecoration: 'underline' }, cursor: 'pointer' }}
                                        color='brand.600'
                                        fontSize="sm"
                ...
                const allColumns = [
                    ...baseColumns,
                    ...customColumns,
                    ...(permission?.update || permission?.view || permission?.delete ? [actionColumn] : [])
                ];
